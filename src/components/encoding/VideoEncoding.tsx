...
const apiToUI = (data: any, camId: string): VideoEncodingData => {
  ...
  const resolutionOptions =
    camId === "cam2" ? resolutionOptionsThermal : resolutionOptionsOptical;
  const resOption = resolutionOptions.find((r) => r.value === resolution);
  const refBitRate = resOption
    ? Math.floor((resOption.minBitrate + resOption.maxBitrate) / 2)
    : 4096;
  ...
  return {
    ...
    subStreamEnabled:
      String(getVal(subPrefix + "VideoEnable", "false")) === "true",
...
const uiToApi = (ui: VideoEncodingData) => {
  ...
  if (ui.videoCodec === "H.264H") {
    codec = "H.264";
    profile = "High";
  } else if (ui.videoCodec === "H.264B") {
    codec = "H.264";
    profile = "Baseline";
  }
  ...
  return {
    ...
    [`${mainPrefix}Profile`]: profile,
    [`${mainPrefix}resolution`]: ui.resolution,
    [`${mainPrefix}FPS`]: ui.frameRate,
    [`${mainPrefix}BitRateControl`]:
      ui.bitRateType === "variable" ? "VBR" : "CBR",
...
const VideoEncoding: React.FC = () => {
  ...
  const currentResolution =
    resolutionOptions.find((r) => r.value === settings.resolution) ||
    resolutionOptions[2];
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-700 pb-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm">
            Configuration du flux principal (MainFormat) et activation du flux
            secondaire.
          </p>
        </div>
        <button
      ...
      <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {(["H.264", "H.264H", "H.264B", "H.265", "MJPEG"] as const).map(
            (codec) => (
              <button
                key={codec}
      ...
      <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
        ...
        <div className="space-y-4">
          ...
          <input
            ...
            onChange={(e) =>
              handleUpdate({ frameRate: parseInt(e.target.value) })
            }
        ...
        {settings.bitRateType === "variable" && (
          <div className="mb-6 p-4 bg-gray-900/50 rounded border border-gray-600">
            ...
            <input
              ...
              onChange={(e) =>
                handleUpdate({ pictureQuality: parseInt(e.target.value) })
              }
              className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer slider-thumb"
            />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-4 bg-blue-900/20 border border-blue-700/30 rounded-lg">
            ...
            <div className="text-2xl font-bold text-blue-400">
              {currentResolution.minBitrate} - {currentResolution.maxBitrate}{" "}
              kbps
            </div>
          </div>
          ...
          <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-600">
            ...
            <input
              ...
              onChange={(e) =>
                handleUpdate({ bitRate: parseInt(e.target.value) || 0 })
              }
      ...
      <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold text-white">
            Intervalle I-Frame (GOP)
          </h2>
          <span className="text-gray-400 text-sm">
            {settings.iFrameInterval} images
          </span>
        </div>
        <input
          type="range"
          ...
          onChange={(e) =>
            handleUpdate({ iFrameInterval: parseInt(e.target.value) })
          }
          className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer slider-thumb"
        />
        <div className="flex justify-between text-xs text-gray-500 mt-1">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full transition-colors relative ${
                settings.watermarkEnabled ? "bg-red-600" : "bg-gray-600"
              }`}
            >
              <div
                className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${
                  settings.watermarkEnabled ? "left-7" : "left-1"
                }`}
              ></div>
            </button>
          </div>
        ...
        <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full transition-colors relative ${
                settings.subStreamEnabled ? "bg-red-600" : "bg-gray-600"
              }`}
            >
              <div
                className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${
                  settings.subStreamEnabled ? "left-7" : "left-1"
                }`}
              ></div>
            </button>
          </div>
