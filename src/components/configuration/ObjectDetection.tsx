...
const ObjectDetection: React.FC = () => {
    ...
    useEffect(() => {
        const restoreState = async () => {
            try {
                ...
                const response = await fetch(
                    `http://${window.location.hostname}:3000/api/detection/state?cameraId=${selectedCamera}`,
                );
                const data = await response.json();
                ...
                if (data.success && data.state) {
                    ...
                    if (trackingStatus === "running") {
                        ...
                        fetch(`http://${window.location.hostname}:3000/api/detection/start`, {
                            method: "POST",
                    ...
                    if (followStatus === "running") {
                        ...
                        if (savedTracking && JSON.parse(savedTracking) !== null) {
                            ...
                            fetch(
                                `http://${window.location.hostname}:3000/api/track/object/${trackId}`,
                                {
                                    method: "POST",
                                    headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({ cameraId: selectedCamera }),
                    ...
                    if (data.type === "tracking_data") {
                        ...
                        if (data.data.objects.length > 0) {
                            ...
                            data.data.objects.forEach((obj) => {
                                ...
                                const objWithTimestamp = {
                                    ...
                                    imageUrl: existing?.imageUrl || `${API_BASE_URL}/api/detection/object/${obj.trackId}/crop?t=${now}`,
                                };
                        ...
                        clearTimeoutRef.current = setTimeout(async () => {
                            ...
                            if (trackingId !== null) {
                                try {
                                    await fetch(
                                        `http://${window.location.hostname}:3000/api/track/stop`,
                                        {
                                            method: "POST",
                                            headers: { "Content-Type": "application/json" },
                                            body: JSON.stringify({ cameraId: selectedCamera }),
    ...
    const enableDetection = async () => {
        try {
            await fetch(`http://${window.location.hostname}:3000/api/detection/start`, {
                method: "POST",
                headers: {
    ...
    const disableDetection = async () => {
        try {
            await fetch(`http://${window.location.hostname}:3000/api/detection/stop`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            if (trackingId !== null) {
                await fetch(`http://${window.location.hostname}:3000/api/track/stop`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                    },
    ...
    const enableTracking = async (id: number) => {
        try {
            await fetch(
                `http://${window.location.hostname}:3000/api/track/object/${id}`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
