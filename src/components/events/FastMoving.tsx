...
interface FastMovingRule {
  id: number;
  name: string;
  color: string;
  sensitivity: number; // 1-10, corresponding to 10m/s - 1m/s trigger speed
  depthCalibrated: boolean; // Whether depth-of-field calibration is configured
  coordinates: { x: number; y: number }[];
}

export interface FastMovingConfig {
  enabled: boolean;
  ...
  fastMovingRules: FastMovingRule[];
}

interface FastMovingProps {
  config: FastMovingConfig;
  onConfigChange: (config: FastMovingConfig) => void;
  onSave: () => void;
}

const FastMoving: React.FC<FastMovingProps> = ({
  config,
  onConfigChange,
  onSave,
}) => {
  ...
  const updateConfig = (updates: Partial<FastMovingConfig>) => {
  ...
  // Calculate actual speed from sensitivity (1-10 -> 10m/s - 1m/s)
  const getSpeedFromSensitivity = (sensitivity: number): number => {
    return 11 - sensitivity; // Sensitivity 1 = 10m/s, Sensitivity 10 = 1m/s
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-white font-medium flex items-center gap-2">
          ...
          Fast-Moving Detection Configuration
        </h3>
      ...
      <div className="bg-violet-500/10 border border-violet-500/30 rounded-lg p-4">
        <div className="flex gap-3">
          ...
          <div className="text-sm text-violet-300">
            <p className="text-violet-200 font-medium mb-2">Fast-Moving Detection</p>
            <ul className="list-disc list-inside text-violet-200/90 space-y-1 text-xs">
              <li><strong>Function:</strong> Detects targets moving faster than set alarm speed using depth-of-field calibration</li>
              <li><strong>Speed Calculation:</strong> Sensitivity 1-10 corresponds to trigger speeds 10m/s - 1m/s</li>
              <li><strong>Prerequisite:</strong> Depth-of-field calibration must be configured first</li>
              <li><strong>Best For:</strong> Sparse targets, no obvious occlusion, overhead installation</li>
              <li><strong>Installation:</strong> Camera should be above monitoring area with optical axis vertical to movement direction</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h4 className="text-white font-medium mb-1">Enable Fast-Moving Detection</h4>
            <p className="text-gray-400 text-sm">
              Detect targets exceeding alarm speed calculated from depth-of-field calibration
            </p>
          </div>
          <button
            onClick={() => updateConfig({ enabled: !config.enabled })}
            className={`relative inline-flex h-8 w-16 items-center rounded-full transition-all ${
              config.enabled ? 'bg-gradient-to-r from-violet-600 to-violet-700' : 'bg-gray-600'
            }`}
          >
            <span
      ...
      {config.enabled && (
        <>
          {/* Depth-of-Field Calibration Warning */}
          {config.fastMovingRules.some(rule => !rule.depthCalibrated) && (
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4">
              <div className="flex gap-3">
                ...
                <div className="text-sm text-yellow-300">
                  <p className="text-yellow-200 font-medium mb-1">Depth-of-Field Calibration Required</p>
                  <p className="text-yellow-200/90 text-xs">
                    Some rules are not calibrated. Depth-of-field calibration must be configured to calculate actual movement speed.
                    Configure calibration in the Rule Setup to enable accurate speed detection.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-6">
            <label className="flex items-center justify-between text-white font-medium mb-3">
              ...
              <span className="text-violet-400 font-bold text-lg">{config.antiDither}s</span>
            </label>
            <input
              type="range"
              min="0"
              ...
              style={{
                background: `linear-gradient(to right, #8B5CF6 0%, #8B5CF6 ${config.antiDither}%, #374151 ${config.antiDither}%, #374151 100%)`
              }}
            />
            <p className="text-gray-400 text-sm">
          ...
          <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-gray-400 text-sm">Define regions and configure speed sensitivity (1-10 = 10m/s - 1m/s)</p>
              </div>
              <button
                onClick={() => setShowRuleModal(true)}
                className="py-2 px-4 bg-gradient-to-r from-violet-600 to-violet-700 hover:from-violet-500 hover:to-violet-600 text-white rounded-lg transition-all font-medium text-sm shadow-lg"
              >
                Draw Rule
              </button>
            </div>
            ...
            {/* Rules List */}
            {config.fastMovingRules.length > 0 && (
              <div className="space-y-2">
                {config.fastMovingRules.map((rule) => (
                  <div
                    key={rule.id}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-gray-400 text-xs mt-0.5">
                          Sensitivity: <span className="text-violet-400">{rule.sensitivity}</span>
                          {' • '}
                          Trigger Speed: <span className="text-green-400">&gt; {getSpeedFromSensitivity(rule.sensitivity)}m/s</span>
                        </div>
                      </div>
                      <button
                        onClick={() => {
                          updateConfig({
                            fastMovingRules: config.fastMovingRules.filter(r => r.id !== rule.id)
                          });
                        }}
          ...
          <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-sm">Automatically record video when fast-moving target is detected</p>
              </div>
              <button
            ...
            {config.record && (
              <div className="pt-4 border-t border-gray-700">
                ...
                />
                <p className="text-gray-400 text-sm">
                  Recording duration after fast-moving detection (10-300 seconds)
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-sm">Send email notification when fast-moving target is detected</p>
              </div>
              <button
          ...
          <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-sm">Trigger PTZ action when fast-moving target is detected</p>
              </div>
              <button
          ...
          <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-sm">Capture image snapshot when fast-moving target is detected</p>
              </div>
              <button
          ...
          <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-6">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-gray-400 text-sm">Configure time periods for fast-moving detection</p>
              </div>
              <button
          ...
          <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="text-gray-300">
                <span className="text-gray-400">Rules Configured:</span> {config.fastMovingRules.length}
              </div>
              <div className="text-gray-300">
                <span className="text-gray-400">Calibrated Rules:</span> {config.fastMovingRules.filter(r => r.depthCalibrated).length}
              </div>
              <div className="text-gray-300">
      ...
      <div className="flex justify-end">
        <button
          onClick={onSave}
          className="py-3 px-8 bg-gradient-to-r from-violet-600 to-violet-700 hover:from-violet-500 hover:to-violet-600 text-white rounded-lg transition-all font-medium shadow-lg transform hover:scale-105"
        >
          Save Configuration
        </button>
      </div>
      ...
      {showRuleModal && (
        <RuleSetupModal
          fastMovingRules={config.fastMovingRules}
          onSave={(rules) => {
            updateConfig({ fastMovingRules: rules });
...
interface RuleSetupModalProps {
  fastMovingRules: FastMovingRule[];
  onSave: (rules: FastMovingRule[]) => void;
  onClose: () => void;
}
...
const RuleSetupModal: React.FC<RuleSetupModalProps> = ({
  ...
  const [rules, setRules] = useState<FastMovingRule[]>(fastMovingRules);
  const [selectedRuleId, setSelectedRuleId] = useState<number | null>(
    fastMovingRules.length > 0 ? fastMovingRules[0].id : null
  );
  ...
  const handleAddRule = () => {
    ...
    const newRule: FastMovingRule = {
      id: Date.now(),
      name: `Rule${rules.length + 1}`,
      color: colors[rules.length % colors.length],
      sensitivity: 5, // Default sensitivity 5 = 6m/s
  ...
  const handleUpdateRule = (id: number, updates: Partial<FastMovingRule>) => {
  ...
  // Calculate trigger speed from sensitivity
  const getSpeedFromSensitivity = (sensitivity: number): number => {
    return 11 - sensitivity; // Sensitivity 1 = 10m/s, Sensitivity 10 = 1m/s
  };
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-lg border border-gray-700 w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="p-6 border-b border-gray-700">
          ...
          <p className="text-gray-400 text-sm mt-1">
            Define regions and configure sensitivity (1-10 = 10m/s - 1m/s trigger speed). Depth-of-field calibration required.
          </p>
        </div>
            ...
            <div>
              ...
              <div className="relative aspect-video bg-gray-900 rounded-lg border-2 border-gray-700 overflow-hidden">
                ...
                {rules.map((rule) => (
                  ...
                  >
                    ...
                    >
                      {rule.name} (&gt;{getSpeedFromSensitivity(rule.sensitivity)}m/s)
                    </div>
                    {!rule.depthCalibrated && (
                      <div className="absolute bottom-2 left-2 px-2 py-1 rounded text-xs font-bold bg-yellow-500/90 text-white">
                        Not Calibrated
                      </div>
                    )}
                  </div>
              ...
              <div className="mt-3 flex gap-2">
                <button
                  ...
                  className="flex-1 py-2 px-4 bg-violet-600 hover:bg-violet-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg transition-all font-medium text-sm"
                >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-gray-400 text-xs mt-0.5">
                          Sensitivity: {rule.sensitivity} • Trigger: &gt;{getSpeedFromSensitivity(rule.sensitivity)}m/s
                        </div>
                      </div>
                      <button
                        onClick={(e) => {
                ...
                <div className="bg-gray-900/50 rounded-lg border border-gray-700 p-4 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-violet-500"
                  ...
                  <div>
                    <label className="block text-gray-400 text-xs mb-2">Depth-of-Field Calibration</label>
                    <div className="flex items-center justify-between p-3 bg-gray-800 rounded-lg border border-gray-600">
                      <div className="flex items-center gap-2">
                        <div className={`w-3 h-3 rounded-full ${selectedRule.depthCalibrated ? 'bg-green-500' : 'bg-yellow-500'}`}></div>
                        <span className={`text-sm ${selectedRule.depthCalibrated ? 'text-green-400' : 'text-yellow-400'}`}>
                          {selectedRule.depthCalibrated ? 'Calibrated' : 'Not Calibrated'}
                        </span>
                      </div>
                      <button
                        onClick={() => handleUpdateRule(selectedRule.id, { depthCalibrated: !selectedRule.depthCalibrated })}
                        className={`px-3 py-1 rounded text-xs font-medium ${
                          selectedRule.depthCalibrated 
                            ? 'bg-yellow-600 hover:bg-yellow-500 text-white' 
                            : 'bg-green-600 hover:bg-green-500 text-white'
                        }`}
                      >
                        {selectedRule.depthCalibrated ? 'Reset Calibration' : 'Configure Calibration'}
                      </button>
                    </div>
                    <p className="text-gray-500 text-xs mt-1">
                      Calibration required to calculate actual movement speed
                    </p>
                  </div>
                  ...
                  <div>
                    <label className="flex items-center justify-between text-gray-400 text-xs mb-2">
                      <span>Sensitivity (Alarm Trigger Speed)</span>
                      <span className="text-white font-bold">
                        {selectedRule.sensitivity} = &gt;{getSpeedFromSensitivity(selectedRule.sensitivity)}m/s
                      </span>
                    </label>
                    <input
                      type="range"
                    ...
                    <p className="text-gray-500 text-xs mt-2">
                      Lower sensitivity = Higher trigger speed | Higher sensitivity = Lower trigger speed
                    </p>
                  </div>

                  {/* Speed Reference Chart */}
                  <div className="bg-violet-500/10 border border-violet-500/30 rounded p-3">
                    <p className="text-violet-200 text-xs font-medium mb-2">Speed Reference Chart:</p>
                    <div className="grid grid-cols-2 gap-1 text-xs">
                      <div className="text-violet-300">Sensitivity 1 → 10 m/s</div>
                      <div className="text-violet-300">Sensitivity 6 → 5 m/s</div>
                      <div className="text-violet-300">Sensitivity 2 → 9 m/s</div>
                      <div className="text-violet-300">Sensitivity 7 → 4 m/s</div>
                      <div className="text-violet-300">Sensitivity 3 → 8 m/s</div>
                      <div className="text-violet-300">Sensitivity 8 → 3 m/s</div>
                      <div className="text-violet-300">Sensitivity 4 → 7 m/s</div>
                      <div className="text-violet-300">Sensitivity 9 → 2 m/s</div>
                      <div className="text-violet-300">Sensitivity 5 → 6 m/s</div>
                      <div className="text-violet-300">Sensitivity 10 → 1 m/s</div>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="bg-violet-500/10 border border-violet-500/30 rounded-lg p-3">
                <div className="text-sm text-violet-300">
                  <p className="text-violet-200/90 text-xs">
                    ...
                    • Camera should be installed above the monitoring area<br />
                    • Optical axis should be as vertical as possible to movement direction<br />
                    • Best for sparse targets with no obvious occlusion<br />
                    • Depth-of-field calibration must be configured first
                  </p>
                </div>
              </div>
            </div>
